...
If the sandbox is not yet in the `Ready` phase (for example, right after onboarding while the 2.4 GB image is still pulling), `connect` polls `openshell sandbox list` every few seconds and prints the current phase so you can see progress instead of a silent hang.
