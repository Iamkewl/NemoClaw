---
title:
  page: "Tutorial: NemoClaw on DGX Spark"
  nav: "DGX Spark"
description:
  main: "End-to-end tutorial for running NemoClaw on an NVIDIA DGX Spark, including optional local inference with Ollama on the Spark GPU."
  agent: "End-to-end tutorial for running NemoClaw on NVIDIA DGX Spark (aarch64, Ubuntu 24.04). Use when installing NemoClaw on a DGX Spark, configuring local inference with Ollama on the Spark GPU, or troubleshooting Spark-specific issues."
keywords: ["nemoclaw dgx spark tutorial", "nemoclaw spark install", "nemoclaw ollama spark"]
topics: ["generative_ai", "ai_agents"]
tags: ["openclaw", "openshell", "sandboxing", "nemoclaw", "dgx-spark", "ollama", "tutorial"]
content:
  type: tutorial
...
# Tutorial for Setting Up NemoClaw on DGX Spark with Local Ollama
...
DGX Spark ships with Ubuntu 24.04 and Docker pre-installed, so no extra pre-setup is required before the standard installer. Refer to [Platform Setup](../platform-setup/index.md) for the full pre-setup matrix across platforms.

## How NemoClaw Runs on DGX Spark
...
For the deployment topology diagram across all platforms, and how this Spark layering maps to them, refer to the [deployment topology section in the Architecture reference](../../reference/architecture.md#deployment-topology).

## Prerequisites

This tutorial assumes you have already completed the [Quickstart](../quickstart.md) and have a working NemoClaw sandbox running on your DGX Spark. Confirm your environment meets the general [Prerequisites](../prerequisites.md) before continuing.

In addition, this tutorial requires the following.
...
No cloud-provider API key is required for this tutorial. Local Ollama replaces the cloud inference path.
...
5. Point your NemoClaw sandbox at Local Ollama. How you do this depends on which provider you picked during the [Quickstart](../quickstart.md).

   - If you picked Local Ollama during the Quickstart, no re-onboarding is needed. Continue to the next step.
   - If you picked a cloud provider and want to change the active model without rebuilding the sandbox, refer to [Switch inference providers](../../inference/switch-inference-providers.md). The command is shown below.
...
Now that local inference is running on your DGX Spark, here are useful follow-ups.

- If something goes wrong, refer to the [DGX Spark section](../../reference/troubleshooting.md#dgx-spark) of the Troubleshooting reference. It covers Spark-specific issues such as CoreDNS crashes, `k3s` image pull failure, GPU passthrough, `pip install` system-packages errors, and the AI Workbench port 3000 conflict.
...
- Try other local inference backends. [Use a Local Inference Server](../../inference/use-local-inference.md) covers vLLM, NIM, and OpenAI-compatible endpoints generically, not Spark-specific.
- Swap models without re-onboarding. See [Switch Inference Providers](../../inference/switch-inference-providers.md).
